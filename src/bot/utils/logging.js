...
const logPath = path.resolve("src/bot/unrecognizedQueries.json");

export const logUnrecognizedQuery = async (query, userId, state) => {
  if (
    !query ||
    query.trim() === "" ||
    userId.includes("status@broadcast") ||
  ...
  if (state.unrecognizedQueries.length > 1000)
    state.unrecognizedQueries = state.unrecognizedQueries.slice(-500);

  try {
    await fs.writeFile(
      logPath,
      JSON.stringify(state.unrecognizedQueries, null, 2)
    );
  } catch (err) {
